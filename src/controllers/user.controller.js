    ...
    try {
      ...
      await user.save({validateBeforeSave: false})
       ...
       return {accessToken,refreshToken}
        ...
        throw new ApiError(500, "somethng went wrong while generating refresh and access token")
    }
}
...
const registerUser = asyncHandler(async (req,res) =>{
    ...
    // other way of writing the same things
...
const refreshAccessToken = asyncHandler(async(req,res)=>{
  ...
  try {
    const decodedToken = jwt.verify(incomingrefreshToken,process.env.REFRESH_TOKEN_SECRET)
     const user = await User.findById(REFRESH_TOKEN_SECRET?._id)
     ...
     if(incomingrefreshToken !== user?.refreshToken){
      throw new ApiError(401, "Refresh Tiken is expired or used")
     }
   ...
   const user = await User.findByIdAndUpdate(
       req.user?._id,
       {
        $set: {
            // fullName: fullName,
            // email: email
            //both the ways are correct we can write using any one of them 
            fullName,
            email
        }
       },
